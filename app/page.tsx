          ...
          <div className="max-w-5xl mx-auto bg-white p-8 md:p-12 rounded-3xl shadow-xl">
            ...
            <div className="mb-12">
              ...
              <div className="grid grid-cols-3 md:grid-cols-4 gap-3">
                {['당일치기', '1박 2일', '2박 3일', '3박 4일', '4박 5일', '5박 6일', '6박 7일', '직접 입력'].map(day => (
                  <button key={day} className={`p-4 border rounded-xl font-semibold transition-colors ${day === '당일치기' ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-300 hover:border-blue-500 hover:bg-blue-50'}`}>
                    {day}
                  </button>
                ))}
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {czechCities.map((city, index) => (
                  <label key={city.name} className="relative group cursor-pointer block rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-all h-40 border border-gray-200">
                    <input type="checkbox" className="absolute top-3 right-3 w-5 h-5 z-20 rounded text-blue-600 focus:ring-blue-500 cursor-pointer" defaultChecked={index === 0} />
        ...
        <div className="min-h-screen bg-gray-50 py-16 px-4">
          <div className="max-w-4xl mx-auto">
            <div className="flex justify-between items-center mb-12 print:hidden">
              <h2 className="text-4xl font-extrabold text-gray-900">✨ 완성된 실시간 맞춤형 프라하 일정</h2>
              <button onClick={() => setStep('setup')} className="px-5 py-2 bg-gray-200 rounded-lg hover:bg-gray-300 font-semibold">다시 설정</button>
            </div>

            <div className="bg-white rounded-3xl shadow-xl p-8 md:p-12 border-t-8 border-blue-600 relative print:shadow-none print:border-none print:p-0">
              <h3 className="text-3xl font-bold text-gray-800 mb-10 pb-4 border-b-2 border-gray-100">Prague One-Day Master Course (AI Generated)</h3>
              ...
              <div className="mt-16 flex flex-col sm:flex-row justify-center gap-4 print:hidden">
